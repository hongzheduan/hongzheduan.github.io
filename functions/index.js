...
app.get("/market-news", async (req, res) => {
  ...
  try {
    ...
    const items = parsed.slice(0, 30).map(it => ({
      title: it.title,
      source: it.source,
      link: it.link,
...
- Complete FAQ title list: "Is Baizora free?", "How does the Free Tier work?", "Do I need to sign up to use Baizora?", "How do I create an account?", "How do I access my dashboard and analysis results?", "Where do I go after logging in?", "How does the 7-day free trial work?", "How is the data updated?", "How current is the P/E and EPS data?", "What do the sparkline markers mean?", "What do the timeframe windows show?", "What are Baizora Scores?", "What stocks does Baizora cover?", "How does Baizora compare to Finviz or TradingView?", "How accurate is the S&P 500 and Nasdaq-100 constituent list?", "Is Baizora a financial advisor or recommendation service?", "Where is my watchlist? My saved stocks are gone.", "How do I subscribe to a plan?", "How do I manage or cancel my subscription?", "What's the difference between monthly and yearly plans?", "How do promo codes work?", "What happens after the free trial ends?", "Can I use the same email address or credit card to get another free trial?", "What is your refund policy?", "I subscribed but can't access the dashboard — what should I do?", "I can't log in — I keep getting an error or being redirected.", "The data looks outdated — it hasn't updated today.", "I never received my verification email.", "The dashboard is blank or not loading."
- "What is Baizora?" or general platform questions → one sentence + [About Baizora](https://baizora.com/assets/about.html).
- "Is Baizora free?" / "Do I have to pay?" / any cost question → answer directly, do NOT point to the Free Tier FAQ (that's a separate no-login preview, not the answer to this question): "Yes — the full Baizora platform is currently free to use, no subscription required. [Sign in](https://baizora.com/login.html) or [create a free account](https://baizora.com/signup.html) to get started."
...
- 完整常见问题条目列表："贝佐拉免费吗？"、"免费版是如何运作的？"、"使用 Baizora 需要注册账户吗？"、"如何创建账户？"、"如何进入个人主页及查看分析结果？"、"登录后会跳转到哪里？"、"7天免费试用是如何运作的？"、"数据是如何更新的？"、"市盈率和每股收益数据有多新？"、"走势图上的标记代表什么？"、"时间周期窗口显示哪些信息？"、"什么是贝佐拉评分？"、"Baizora 覆盖哪些股票？"、"贝佐拉与 Finviz 或 TradingView 相比如何？"、"标普500和纳斯达克100成分股名单的准确性如何？"、"Baizora 是投资顾问或推荐服务吗？"、"我的自选股去哪了？收藏的股票消失了。"、"如何订阅方案？"、"如何管理或取消我的订阅？"、"月付和年付方案有何区别？"、"优惠码是如何生效的？"、"免费试用结束后会发生什么？"、"我可以用相同的邮箱或信用卡再次获得免费试用吗？"、"退款政策是什么？"、"我已订阅但无法访问个人主页，该怎么办？"、"我无法登录——一直报错或被跳转。"、"数据看起来过期了，今天还没有更新。"、"我没有收到验证邮件。"、"个人主页空白或无法加载。"
- "贝佐拉是什么？"等平台总体问题 → 一句话 + [关于贝佐拉](https://baizora.com/assets/about_cn.html)。
- "贝佐拉免费吗？"、"需要付费吗？"等费用相关问题 → 直接回答，不要引导至"免费版是如何运作的？"（那是另一个无需登录的预览功能，不是这个问题的答案）："是的——贝佐拉目前对所有用户完全免费，无需订阅。[登录](https://baizora.com/login_cn.html)或[免费注册](https://baizora.com/signup_cn.html)即可开始使用。"
