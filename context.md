...
## What Was Done 2026-07-08/09 — "Baizora Report" Daily Market-News Short: IN PROGRESS, not yet wired into production

**Goal:** a new daily video category — a "reporter" mascot reads out the day's real market news headlines — generated in EN+CN alongside (weekdays) or instead of (weekends) the existing category Shorts, with its own homepage download button. This spans two sessions (started 07-08, still going 07-09); nothing below is committed to git yet and the pipeline is not yet wired into `scanner.yml`.

**Design iterated live, several rejected turns before landing:**
1. Background: a fully procedural PIL skyline was tried and discarded once the user supplied `video/covering/news_background.png` (a real 2×2 grid of Wall Street photos, same convention as the weekday cover art) — split into `News_1.png`–`News_4.png`. Went through full-bleed-photo (looked great but 2 of the 4 photos have their own bold embedded text — the "WALL ST" sign, a stock-ticker readout board — that collided with our headline text) before landing on the current layout: a compact solid-navy text band (`TEXT_BAND_H = round(SH*0.25)`, top 1/4) with the real photo filling the remaining 3/4, unobstructed. **One photo per whole video, day-rotated** (`date_obj.toordinal() % 4 + 1`), not cycled per-scene within a video — user was explicit about this ("i provided four because each day we can use different one").
2. Mascot: went through a procedural fox (rejected — half the head, mouth got covered by the body layer, several fixes), then `video/mascot_sample.png` (a teal AI-mascot with a "NEWS" mic, tried hybridizing with the fox head — messy), then `video/mascot_sample2.png` (a bull reporter — good quality but not the actual brand character), before landing on **`video/mascot_sample4.png`** — the real Baizora/白泽 (Baize) mascot the user supplied, which is what ships. Processing applied: background keyed via flood-fill-from-corners (preserves enclosed white areas like the shirt — plain color-distance keying punched holes in the white dress shirt since it's the same near-white as the background); the "白泽" calligraphy + red seal removed via a **bounded box mask** (`y:180-700, x:0-230`), not a full-height column — an earlier full-column cut sliced through the character's actual tail, which curls down near the feet around y:800-1400, same x-range as the calligraphy; the mic's "NEWS" flag relabeled to **"Baizora"** via a perspective-transform warp (`PIL.Image.transform(..., Image.PERSPECTIVE, coeffs)`, coefficients solved with the standard `find_coeffs(pa, pb)` recipe — get the pa/pb argument order right, source-quad-first, or the warp silently produces an empty/transparent result) matching the mic's tilt angle exactly, then the quad expanded ~12% outward from centroid so no sliver of the original "S" peeked out at one corner. Final asset: **`video/baize_mascot.png`** (RGBA, transparent bg). Animation: since it's one static pose (no alternate mouth frames), a subtle scale-pulse ("breathing", not lip-sync) stands in — `amplitude=0.015`, `cycle_frames=48` — user said an earlier `0.045`/`24-frame` version "moved too much".
3. Duration: started at 3 stories / ~27s (matching the existing weekday Shorts length), user asked for up to ~90s to cover **more real stories**, not padding — landed on up to 10 distinct stories, whatever a given day's news genuinely supports (not forced to a fixed count). Real find while picking stories: **must dedupe near-duplicate headlines about the same event** (today's Iran-ceasefire selloff was independently reported by 4+ outlets) and **must exclude generic listicle/opinion content that isn't a real news event** — e.g. Investopedia's "5 Things to Know Before the Market Opens", Motley Fool's "History Says This Will Happen Next" — the user caught the first hand-picked draft including one of these and flagged it as "actually not a news [event]".
4. Narration generation (the hardest open question): the approved sample's narration was hand-paraphrased by the assistant in-chat with real added context ("for a second straight day", etc.) — not reproducible by a template script running unattended every day. Two options were costed out: algorithmic (dedupe via keyword-overlap clustering + regex listicle filter + template-wrap the literal headline — free, but sounds like a headline ticker) vs. an LLM call (Claude Haiku 4.5 paraphrasing genuinely, matching the demo's style). **User chose the LLM option** after seeing real numbers: ~$0.007–0.02/day (~$0.20–0.60/month) for ~10 headlines × 2 languages × 1 call each — negligible. Discovered along the way: the existing chat-widget backend (`functions/index.js`) **already uses the Anthropic SDK and already has a secret named `ANTHROPIC_API_KEY`** — but it's a Firebase Functions secret, a different store than GitHub Actions secrets. User has now added the same key value as a **GitHub Actions repo secret** too ("done option 1" — reusing one key across both features rather than minting a second one, given the negligible cost).

**What's actually built (uncommitted, in the working tree):**
- **`video/generate_news_report.py`** (new, production-quality, not scratch) — loads `data/market_news.json` / `market_news_cn.json`, calls `claude-haiku-4-5` with `output_config: {format: {type: "json_schema", ...}}` for structured `{headline, narration, source}` output per story (one system prompt each for EN/CN handling dedup + listicle-filtering + paraphrase together in one call — no separate algorithmic dedup step was needed once the LLM path was chosen), renders hook → up to 10 story cards → outro via the same `generate_video.encode()` used everywhere else. If the Claude call raises for any reason, the day's report is skipped (prints a message, writes no file) rather than crashing the run — same pattern as other categories' "not enough qualifying items" skip.
- **Duration estimator, calibrated not hand-measured** (content is dynamic per day, can't pre-measure by hand like the fixed weekday templates): fit via linear regression over ~24 real `edge-tts` measurements taken this session — `estimate_duration_en(text) = 0.218*len(text.split()) + 2.4` (words), `estimate_duration_cn(text) = 0.132*len(text) + 1.6` (chars) — both include a safety buffer over the fitted line's worst-case residual (~0.6-0.7s).
- **`video/baize_mascot.png`** — final mascot asset (see above).
- **`video/covering/News_1.png`–`News_4.png`** — split from user-supplied `news_background.png`.
- **`video/requirements.txt`** — added `anthropic`.
- Smoke-tested structurally with a stubbed `curate_stories()` (real render, both EN and CN, correct frame counts, clean encode, no narration-cutoff warnings) — **the real Haiku API call itself has not been tested yet**, no key available in this local sandbox; first real test will happen via GitHub Actions or a manual run.

**Not done yet — pick up here next session:**
1. **Wire into `scanner.yml`**: weekdays should generate **4 videos total** (existing EN+CN category Short, unchanged, + new EN+CN news report) at the same 4:30 PM trigger. Weekends need a **new cron** (~4 PM ET, i.e. `"0 20 * * 0,6"` matching the existing UTC-offset convention used for weekday crons) that generates **only** the EN+CN news report (no category video, no full market scan needed — weekends currently only run EDGAR-only). `upload_youtube.py`'s type dispatch needs a new `news_report`/`news_report_cn` entry (title/description templates).
2. **Homepage download buttons**: add a second download link to `index.html` and `index_cn.html` (parallel to the existing single "Latest video" link in the footer) pointing at new `data/latest_video_news_en.mp4` / `_cn.mp4` + a new `data/latest_video_news_meta.json` (mirroring the existing `latest_video_meta.json` structure) — each homepage in its own language.
3. Once scanner.yml is wired, trigger a real run (workflow_dispatch or wait for schedule) to confirm the actual Claude Haiku call works end-to-end in production (structured output parses, narration reads naturally, no rate-limit/auth surprises).
4. Housekeeping: `video/_mascot_preview_TEMP.png` (untracked scratch preview, safe to delete once no longer needed) and `video/mascot_sample.png`/`mascot_sample2.png`/`mascot_sample3.png` (already-tracked earlier reference images, no longer used — `mascot_sample4.png` is the one that shipped) are still sitting in the repo; worth a cleanup pass once the feature ships, not blocking.
...
1. Ad reel cut from 3 parts (dashboard still → screen recording → ticker-detail still) down to just the platform pitch spoken as a bare CTA.
2. The ticker-detail-modal still (PANW candlestick card) removed entirely — not kept as a background, no image asset in the ad section at all anymore.
3. The whole ad now lives on the video's actual final slide — a copy of `scene_ad_short()` (the brand outro card), not a separate slide — narrated with just `"baizora点com"`.
4. Its disclaimer line swapped from the usual short one to the requested wording: `本内容仅供数据分享与学习交流，不构成任何投资建议` — shown on-screen only, deliberately not included in the spoken narration (user was explicit about this).

**Real production bug found and fixed while building this test (commit not yet made):** `generate_shorts.py`'s `scene_ad_short()`, line ~406, drew the CN date line (`f"每日扫描：{scan_date}"`) with `load_font(18, mono=True)` — the DM Mono bundled font has no CJK glyphs, so those characters render as tofu boxes. Confirmed on **both** the scratch test render **and** the real already-published `data/latest_video_cn.mp4` (today's actual production output, checked frame-by-frame) — every CN Short's outro card has been rendering this one line broken since the Shorts pipeline shipped. User didn't notice it on their own playback (small, dim, bottom-of-frame, only visible for the last ~2s), but it reproduces consistently on this machine. **Fixed** to `load_font_cn(18)` for the CN branch, matching every other CN line on that card. This fix is applied directly in `video/generate_shorts.py` but **not committed** — small, isolated, easy to ship whenever, doesn't need to be bundled with the ad-section experiment above.

**Where the test output lives:** `video/test_best_performer_cn_adtest.mp4` (gitignored via `video/.gitignore`'s `*.mp4`, confirmed not staged) + the build script at the session scratchpad path (`test_ad_cn.py`, not in the repo). Rebuilt several times chasing a rendering artifact — the first attempt at swapping the disclaimer text left a faint ghost of the old default disclaimer's descenders showing through (blanking rectangle band was too short, `SH-280` to `SH-240`); widened to `SH-290`–`SH-225` and the artifact is gone, confirmed via extracted frames at two different timestamps.

**Not yet applied anywhere else — explicitly deferred:** this ad-section simplification (single-slide, bare-CTA, no product-still ad) currently exists **only** as the one-off CN `best_performer` (Tuesday) test build. User said they *may* want to apply the same treatment to the other 4 weekday builders later, but explicitly **not now** — don't proactively roll this out to Monday/Wednesday/Thursday/Friday or to EN without being asked again.

---

## What Was Done 2026-07-07 — Thursday/Friday Cover Art Split + Monday Title Wording Was Fixed in Only 1 of 3 Places + Stock Pages' Wrong "15-Min Delayed" Label (all shipped)

**Thursday/Friday cover art (commit `3658913`):** user supplied `video/covering/Thursday.png` + `Friday.png`, each a 941×1672 2×2 grid of 4 real designs, to replace the last two weekdays still running Monday-placeholder covers (the 07-05 entry below had incorrectly assumed this gap was already closed). Each quadrant was already ~470×836 — almost exactly the existing 466×832 target aspect ratio — so a minimal center-crop worked cleanly (same safe case as Tuesday's original grid, not Wednesday's edge-to-edge square problem that had sliced "PULL-BACK" into "LL-BACK"). Previewed the reassembled 2×2 grid before trusting the batch; no text was cut off. All 5 weekdays now have real per-weekday cover art.

**Monday's wrong title wording turned out to exist in three independent hardcoded copies, not one (commits `6d1017c`, `0e51929`):** the 07-05 fix below only patched `upload_youtube.py`'s `make_meta()` (the YouTube upload's title metadata). User reported that the *uploaded title* was correct but the *video itself* still opened on the old "TOP VOLUME STOCKS TODAY" wording — traced to a second, completely separate hardcoded string in `generate_shorts.py`'s `build_volume_spikes_short()`, the literal text baked into the hook scene (slide 1) of the rendered video: `scene_hook_generic(date, lang, ["TOP VOLUME", "STOCKS TODAY"], ["今日成交量", "最高股票"], ...)`. Fixed to `["BIGGEST VOLUME", "SPIKES TODAY"]` / `["今日成交量", "暴涨股票"]`, verified by rendering both EN/CN hook frames standalone. A repo-wide grep afterward turned up a *third* copy: `generate_thumbnail.py`'s `thumb_volume_a()` headline, `["TOP VOLUME", "STOCKS"]` — this file is superseded by the real per-weekday cover art and isn't called by the automated pipeline (only reachable via its own CLI), but the wrong text was still sitting there live; fixed to `["BIGGEST VOLUME", "SPIKES"]` and rendered standalone to confirm no overflow. No further occurrences found — `scanner_tiingo.py`'s briefing text, `data/daily_briefing.txt`, and `generate_video.py`'s narration were all already correctly framed around "spikes," not raw volume ranking. **Lesson: this wording existed in three fully independent hardcoded copies (YouTube title metadata, the video's own baked-in hook text, and the standalone thumbnail tool) with no shared source string — any future title/wording fix for a Shorts category needs to grep the whole `video/` directory, not just `make_meta()`.**

**Stock pages wrongly labeled live prices "15-min delayed" (commit `47424a3`):** user asked whether individual stock pages (`stocks/{TICKER}.html`, e.g. `stocks/MSFT.html`) need a 15-min-delay disclaimer. Investigated what data actually powers their live price refresh: both the stock pages and the main dashboard call the same `/iex-quotes` Cloud Function (`functions/index.js`), which in turn calls **Tiingo's** `/iex/` REST endpoint — confirmed via Tiingo's own docs to be real-time IEX TOPS data (top-of-book + last sale), not a delayed feed. The main dashboard already correctly labels this "live" (green `.live-tag`). Only the stock pages had it wrong, hardcoding `"Prices 15-min delayed · As of {t} ET"` in `refresh()`. This must not be confused with the unrelated, paused `project_iex_backup_candidate` investigation (raw IEX Exchange data pulled directly, not via Tiingo, which genuinely *is* 15-min-delayed per IEX's own free-tier policy) — that data was never shipped to production. Fixed the template string in `generate_stock_pages.py` (~line 182) to `"Live price · As of {t} ET"` and scripted the same replace across all 22 existing `stocks/*.html` pages rather than waiting on the next scanner-triggered regen.

**All three commits pushed cleanly to `main`, no rebase conflicts** (`d2f7063` → `47424a3`, on top of the Thursday/Friday cover commit `3658913` and routine automated news-refresh commits already on `origin/main`).
...
New vertical (1080×1920, 9:16, <30s) companion to the existing landscape daily videos in `generate_video.py`, covering all five weekday categories in both EN and CN. Committed and pushed (`b06f21a`); preview `.mp4` outputs are intentionally gitignored (`video/.gitignore`, `*.mp4`) — local-only, not committed.

**Architecture, built incrementally Monday→Friday:**
- Shared scene functions across weekdays: `scene_hook_generic` (title card with per-category background — bar-chart texture, real photo/illustration composited via a brightness-based alpha cutout for flat-black source images, or procedural candlesticks), `scene_top5_table` (condensed ranked list, row count adapts 1-5 for sparse categories), `scene_trend_short` (1-year or rotating-window price chart), `scene_dashboard_short` (real dashboard screenshots), `scene_ad_short` (closing card).
- **Measure-don't-guess TTS timing**: every narration line's actual `edge-tts` duration was measured via small throwaway scripts before locking in `hold_sec` values — guessed durations were the single biggest recurring bug all week (narration cut off mid-word). `generate_narration()` in `generate_video.py` now prints an explicit warning with the exact overflow amount when a clip runs long, which made this diagnosable.
- **Real Baizora icon on every ad card** (`assets/baize_favicon_v2.png`, cut out from its flat-black background via `_load_logo_cutout()`) replacing the old procedural bar-chart texture + hand-drawn wordmark — user's explicit request, applied to all 5 builders.
- **Per-category hook art** (all real user-provided images, not procedural): `bull.png` (Tuesday, bull market), `highest.png` (Wednesday's original pick, later reused for Thursday), `bounceback.png` (Wednesday's final pick — pullback-then-recovery V-shape; also briefly used for Thursday before being corrected off it), `memberchange_slide1.png` (Friday). `_paste_cover()` scales+crops to cover the hook region (like CSS `object-fit: cover`); added a `valign` param after `bounceback.png`'s arrow tip got sliced off by a center-crop — arrow-focused images now crop from the bottom, keeping the top intact.

**Per-weekday notes:**
- **Monday** (`volume_spikes`) — top-5 by `VolumeChange1D`. Straightforward; first one built, established the whole architecture.
- **Tuesday** (`best_performer`) — top-5 by the Tuesday rotating window (`TUESDAY_TF_ROTATION`, 2W→1Y by ISO week). Hook uses `bull.png`.
- **Wednesday** (`6m_breakout`) — **real bug found and fixed**: `WEDNESDAY_TF_ROTATION`'s `min_drawdown` was `10` for all 5 rotation windows, but a leftover legacy comment (`_compute_6m_breakout()`, now-dead code) documented the *original* spec as "20% drawdown" — confirmed via git-archaeology that 10% was a regression introduced when the single-window logic was generalized into the 5-window table. Fixed back to 20 in `generate_video.py` (affects both the landscape build and this Short). Also: earlier narration versions ranked by today's `PriceChange1D` and never stated the actual pullback percentage or which timeframe was active (it rotates weekly) — both were the actual story for this category and were silently missing. Fixed by ranking by `_drawdown` instead, displaying it as a signed red percentage in the table, and having every ticker line explicitly state "pulled back N percent, then broke out to a new {label} high." Only 3 (not 5) tickers are narrated since stating the pullback % per line runs longer than Monday/Tuesday's plain cadence.
- **Thursday** (`1y_vol_peak`) — volume-record category (today IS a stock's biggest-volume day in the rotating window). **Went through a wrong turn**: initially framed with "rebound"/bounce-back language and `bounceback.png`, per a misunderstanding that this should mirror Wednesday's pullback story — corrected once the user clarified there's no such thing as a "volume bounce back," only a *price* one (that's Wednesday's job). Reverted to pure volume-record framing, hook art changed to `highest.png`. This category is genuinely rare (most days: 1-3 qualifying stocks, sometimes exactly 1) — `scene_top5_table`'s footer position and vertical centering, and the ticker-narration "last item" flourish, all had to be made row-count-aware rather than assuming 5 (an existing bug pattern in the Tuesday/Wednesday narrate helpers that would have silently misfired here). When exactly 1 stock qualifies, narration explicitly says "There's only one today — TICKER, ...".
- **Friday** (`index_spotlight`) — single-stock feature (member chosen at random from `_get_verified_members()`, matching the landscape build's own random choice), narrated across two frames sharing one spotlight image (join info, then performance since joining — handles both positive and negative cases). Uses dedicated screenshots (`EN/CN_memberchange1/2.png`, the "Index Membership News & Records" and "Membership Changes" pages) instead of the generic dashboard screenshots used by other weekdays, per explicit instruction to keep mentioning that specific feature by name in the CTA. Needed two follow-up pacing fixes: (1) the whole video's duration budget left almost no slack (~0.15s margin vs. ~0.3-0.5s on other days), making the narrator sound rushed — fixed by adding time to the ad cluster; (2) the CTA was one run-on comma sentence ("...complete records, subscribe and visit...") — split into two separate TTS clips with a genuine ~1-second silence between them, using the actual placement mechanism in `generate_narration()` (a frame's `hold_sec` only needs to exceed its own clip's speech duration for the *next* clip's start to shift later by the excess) rather than anything hacky.

**Also shipped:** `generate_thumbnail.py` (10 procedural PIL cover-art templates, mostly superseded by the real per-weekday image assets above) and `video/covering/<Weekday>_<1-4>.png` (4-image-per-weekday cover rotation, keyed by calendar day, embedded as the video's poster frame via `_embed_cover()`). Only Monday/Tuesday/Wednesday/Thursday/Friday's covers are real designs now — all 5 are filled in (earlier assumption that Tuesday-Friday were still placeholders is outdated).

**Verification:** every one of the 10 final builds (5 weekdays × EN/CN) rendered with zero narration-truncation warnings and was spot-checked frame-by-frame (hook/table/trend/screenshot) for correct data, correct CJK rendering, and correct image compositing.

### `isMarketOpen()` holiday-check fix — client-side, 30 files (shipped, live, commit `4ac688a`)

**Symptom (reported same day, market-holiday Friday 2026-07-03, observed July 4th closure):** dashboard kept fetching live IEX prices all day, the "live" column tag stayed on, `PriceChange1D` showed 0%, and the candlestick chart got a duplicate/spurious "today" bar.

**Root cause:** `isMarketOpen()` (client-side JS, present in `baizora_main_form.html` + every other dashboard/stock-page variant) only checked weekday + time-of-day — no NYSE holiday awareness at all, unlike the Python backend's `is_market_holiday()` in `scanner_tiingo.py`. All three symptoms trace to this single gate: `refreshDashPrices()` (fires every 10s via `setInterval`, plus at page-load and a couple of other trigger points) is only ever called from behind `isMarketOpen()`, and it's the same function that (a) hits the IEX cloud function, (b) drives the "live" tag rendering, and (c) appends a synthetic "today" candle whenever the scanner's last real data date doesn't match today (which is always true on a holiday, since the scanner never runs).

**Fix:** ported a holiday-date set (2025-2028, generated from `nyse_holidays()` in `scanner_tiingo.py` to guarantee consistency with the backend) into every affected file via a scripted patch — 30 files total (`baizora_main_form.html` + `_cn`/`_free`/`_free_cn` variants, `baizora_yfinance_beta.html`, `index.html` + `_cn`, `top-price-movers.html`, `unusual-volume.html`, and all 21 individual `stocks/*.html` pages). Verified with `node --check` on every affected `<script>` block (zero syntax errors) plus a logic simulation confirming 2026-07-03 evaluates as closed while normal trading days are unaffected.

**Maintenance note:** the embedded holiday list only covers 2025-2028 — regenerate via `nyse_holidays()` (same one-liner used for this fix) before it runs out.

---

## What Was Done 2026-07-02/03 — scanner_yfinance.py Emergency Backup Scanner (shipped, live)

### The Idea

Separate, simpler backup-insurance idea from the IEX investigation below: if Tiingo ever becomes unavailable, `scanner_yfinance.py` can generate a dashboard data file sourced from `yfinance` directly instead — meant to buy time, not replace Tiingo permanently. No calibration, no raw-protocol parsing — just a straightforward parallel scan using a data source already proven reliable in this codebase (`compare_with_yfinance()`, `scanner_tiingo.py:2693`, already calls yfinance for all ~600 tickers daily, unconditionally, for diagnostic comparison logging).

**Status: shipped and live** (commit `30c6daa`, pushed to `main`). Not yet verified against a real same-day run — pushed just before tonight's `11PM-full` cron, so tomorrow is the first real side-by-side comparison.

### What shipped

- **`scanner_yfinance.py`** — rewrote the old pre-Tiingo legacy file of the same name (previously wrote directly to `data/latest.json` using slickcharts-scraping; that code is gone). New version writes `data/latest_yfinance.json` in the exact schema of `data/latest.json`. Key design choices:
  - OHLCV fetched via `yf.download()` in chunks of 60 tickers with retry/backoff (Yahoo throttles at volume) — tested end-to-end, 516/516 tickers succeeded on first attempt, no retries needed.
  - `auto_adjust=True` means yfinance handles split/dividend adjustment itself — no split-detection machinery needed for prices.
  - Fundamentals (EPS/Sector/CompanyName/SharesOutstanding) are read **read-only** from `data/fundamentals_cache.json` (EDGAR-sourced, written by the normal `scanner_tiingo.py` pipeline) rather than re-fetched from EDGAR — much less code, but a known v1 gap: if Tiingo dies for good, this cache stops refreshing too, even though EDGAR itself would still be reachable.
  - Reuses (duplicated, not imported — `scanner_tiingo.py` has module-level side effects) `calculate_period_metrics()`, the MA21/1-day-delta block, Spark1Y/Beta/Volatility30D block, and all scoring formulas verbatim — pure pandas math, source-agnostic.
  - No per-ticker staleness exclusion (`partialUpdate`/`staleTickers` always `False`/`[]`) and BRK-B market cap falls back to the cached `TiingoMarketCap` figure rather than a live dual-class-share calc — both explicitly documented as v1 simplifications in the file's own docstring.
  - Own disk cache at `data/ohlcv_yfinance_cache/<TICKER>.json` (same shape as the Tiingo cache, separate directory, gitignored).
- **`baizora_yfinance_beta.html`** — an exact clone of the full authenticated dashboard `baizora_main_form.html` (same Firebase auth + subscription check, same UI, same watchlist/search/every feature). Explicitly **not** based on the freetier template — that was tried first and reverted because freetier disables watchlist/search/membership-tab (upsell-gated), which isn't what's wanted for an internal verification tool. Differs from `baizora_main_form.html` by exactly two lines: both `fetch("data/latest.json"...)` calls point at `data/latest_yfinance.json` instead. Not linked from any nav; excluded via `robots.txt`.
- **`scanner.yml`** — new top-level switch `RUN_YFINANCE_BACKUP_SWITCH: "1"` (in the `env:` block) gates a new "Run yfinance backup scanner (testing)" step, wired with `continue-on-error: true` so a yfinance hiccup can never break the real Tiingo pipeline. Runs on the `11PM-full` scheduled run and on manual `workflow_dispatch`. Its output (`data/latest_yfinance.json`) is folded into the existing backup/rebase/restore commit pattern alongside the other data files. Flip the switch to `""` to disable without touching any other logic.
- `scanner_tiingo.py` itself was never touched.

### Verification done so far

Ran `scanner_yfinance.py` locally once (before the switch was wired into `scanner.yml`): schema-diffed field-for-field against a real `data/latest.json` — identical keys, no missing/extra fields. Spot-checked AAPL/MSFT/NVDA — Price/Volume/PE/Beta/Sector/CompanyName all sane; the two runs differed by one calendar day (local `data/latest.json` hadn't been refreshed since 2026-07-01 in this dev checkout, while the yfinance run naturally pulled the true latest close) — not a data-quality issue, just a same-day-comparison artifact to be aware of when doing a real accuracy check later.

**Next step, deferred to "come back and check tomorrow":** compare `data/latest.json` vs `data/latest_yfinance.json` for the *same* trading day, once both have been generated by tonight's `11PM-full` run.

---

## What Was Done 2026-07-02 — IEX-Calibrated Volume Estimation: Investigated, Paused (no code shipped)

### The Idea

Explored building a free backup/insurance volume data source using IEX Exchange's free HIST historical data (raw TOPS/DEEP pcap files), calibrated against Yahoo Finance (`yfinance`, internal use only, never stored/displayed raw) to correct for IEX's well-known ~1-2% market-share volume gap. Plan: `scanner_iex.py` (fully separate from `scanner_tiingo.py`, zero changes to production) would produce `data/latest_iex.json` in the exact schema of `data/latest.json`, surfaced only on a new unlisted `baizora_iex_beta.html` page (banner-marked "testing result", excluded via `robots.txt`, no nav links). Full architecture written to a plan file (`C:\Users\hongz\.claude\plans\sunny-soaring-ladybug.md`) — components: `iex_config.py`, `iex_feasibility_check.py`, `iex_downloader.py`, `iex_hist_parser.py`, `iex_backfill.py`, `iex_daily_update.py`, `iex_calibrator.py`, `iex_volume_estimator.py`, `iex_validator.py`.

**Status: paused after Phase 0 (feasibility) investigation — user is thinking it over before deciding whether to continue.** Two small files were created and left uncommitted in the working tree: `iex_config.py`, `iex_feasibility_check.py`, `requirements_iex.txt` (a separate requirements file so the IEX pipeline never touches the main `requirements.txt`/`scanner.yml` install step), plus a `.gitignore` addition for `data/iex_scratch/`. No other files were touched — `scanner_tiingo.py`, `scanner.yml`, and all existing dashboard/data files are untouched, as designed.

### Key Findings (all empirically verified, not just read from docs)

**IEX HIST availability — better than expected, but oversized:** `https://iextrading.com/api/1.0/hist?date=YYYYMMDD` is a real, live, unauthenticated JSON endpoint (returns direct googleapis.com download links). IEX's own docs say "trailing twelve months" retention, but that's stale — verified live data still exists back to **2017-05-15** (HIST's actual launch day). However, per-day TOPS-feed file sizes have grown ~20x since 2017: ~0.5GB/day (2017-2018) → ~7-12GB/day (2025-2026). A full 2017-present TOPS-only backfill would be **~9TB**, not the originally-assumed ~115GB. Scoped down to a ~90 trading-day backfill (~700-900GB) as a compromise.

**Parse speed is the real killer, not disk/bandwidth:** `iex-parser` (real PyPI package, confirmed working, supports `TOPS_1_6`) decodes IEX-TP binary messages in pure Python — measured rate on this machine: **~1.18MB of compressed input per second, single-threaded**. That means ~9.2 days continuous parsing for a 90-day backfill, ~44 days for 2 years. Tried three fixes, none helped:
- **Npcap** (installed via `winget install WiresharkFoundation.Wireshark`'s dependency, or standalone from npcap.com) — no effect. It accelerates live packet capture from a NIC, not reading an already-saved `.pcap` file — irrelevant to this workload.
- **Raw `.pcap` vs `.pcap.gz`** — no meaningful difference (gzip decompression itself is fast; it was never the bottleneck).
- **tshark** (Wireshark CLI, installed via `winget install WiresharkFoundation.Wireshark`) full-dissection (`-z io,phs`) on the same file — took as long or slightly longer than the Python parser, likely due to default name-resolution overhead. Killed before exhausting all tshark variants (leaner `-n` flag, `capinfos`), since the trend already showed no win.
- **Bottleneck is confirmed to be `iex-parser`'s own per-message Python dict construction** — not I/O, not decompression, not the capture-driver layer. The only remaining levers: parallelize across this machine's 8 cores (~6x, well-understood), or hand-write a custom decoder (untested, uncertain payoff).

**Structural ceiling independent of parse speed — this is what actually stopped the investigation:** IEX HIST is strictly T+1. Empirically confirmed by querying "today" (2026-07-02) vs "yesterday" (2026-07-01) against the live API — today returns `Not Found`. Decoded the GCS upload timestamp of yesterday's file: published **2026-07-02 04:39:58 UTC = 12:39 AM ET**, i.e., **~8.5 hours after** the 4pm close, on the calendar day *after* the trading day. Adding parse time on top (~2.4 hours for one day's file at the measured rate), the earliest a given trading day's IEX-derived volume could ever be ready is **~3am ET the day after** — compare to Tiingo's same-day ~6-7pm delivery. This is a full calendar day behind, structurally, no matter how fast parsing gets. Fine for "yesterday's data, ready before today's open" but can never match same-day delivery. This is likely because Tiingo's/similar vendors' EOD numbers come from an already-aggregated end-of-day summary (or continuous intraday aggregation on dedicated infrastructure), not from batch-reparsing a raw tick dump the next day the way this approach would have.

**Searched for a simpler free alternative to raw HIST pcap parsing — no shortcut exists:**
- IEX's own `/stats/` page — dead, frozen since 2021 (footer copyright even says 2023), and was Retail-Liquidity-Program-specific anyway, not general volume.
- IEX's historical Rule 4552 daily-volume-by-symbol disclosure — real, but was specific to IEX's pre-2016 era as an ATS ("dark pool"); no confirmed modern equivalent since it became a full exchange in 2016.
- FINRA Daily Short Sale Volume Files (`https://cdn.finra.org/equity/regsho/daily/[CODE]shvol[YYYYMMDD].txt`, e.g. `CNMS` = consolidated) — real, free, public, dead simple CSV-like format, no auth. But confirmed scoped to **off-exchange (TRF/ADF/ORF) volume only** — excludes on-exchange trades (IEX, NYSE, Nasdaq lit executions) entirely. Not usable for our purpose.
- **Polygon.io free tier** — mechanically excellent (one API call/day via "grouped daily bars" returns OHLCV+volume for the entire US market, likely real SIP-consolidated volume), but free-tier ToS explicitly restricts to "personal, non-business, non-commercial" use and forbids powering "an application intended for use by end users other than themselves" — doesn't cleanly fit even an internal-only page for a commercial product like Baizora. Same structural issue likely applies to Finnhub/Alpha Vantage/Twelve Data free tiers (unverified in detail, but same business model).
- **Databento** — sells already-parsed historical data (including IEX TOPS, and apparently real consolidated volume) with a one-time **$125 signup credit** (expires 6 months, not recurring/free-forever). Floated as a way to shortcut just the historical backfill (not ongoing use) — not pursued further before the pause.
- Realization worth keeping: IEX's free-and-commercially-clean delayed-data licensing (see `project_iex_backup_candidate.md`) is unusual among data sources — most "free tier" competitors restrict to personal/non-commercial use specifically because their business model is converting free users to paid. That's *why* the original plan targeted IEX despite its low volume coverage, not an oversight.

### Open Questions for Next Session

Nothing is committed to git besides the harmless scaffolding files listed above. If this comes back:
1. Does the ~1 calendar-day-behind ceiling still make this worth building at all, given it can never match Tiingo's same-day delivery? (User was reconsidering this exact point when the session paused.)
2. If yes: parallelize `iex_backfill.py` across 8 cores (~1.5 days for 90-day backfill) vs. use Databento's one-time credit to shortcut backfill specifically.
3. Full detailed architecture/schema-mapping/build-order still lives in the plan file `C:\Users\hongz\.claude\plans\sunny-soaring-ladybug.md` — read that first before re-deriving anything.
...
14. **YouTube Shorts — not yet uploaded.** All 10 preview builds (5 weekdays × EN/CN) are done, verified, and committed (`generate_shorts.py` etc.), but nobody has actually watched a rendered `.mp4` end-to-end yet — only individual frames were spot-checked. Before wiring into `scanner.yml`'s daily upload step, do at least one full playback per weekday to confirm pacing/audio actually feels right, not just that ffmpeg didn't warn.
15. **isMarketOpen() holiday fix — watch the next real holiday.** Fixed and logic-simulated today, but the *actual* end-to-end behavior (no IEX fetch, no live tag, no duplicate candle) hasn't been observed live in a browser on a real holiday yet — 2026-07-03 (today) is the first test case. Confirm on the next one (next NYSE holiday per the embedded list) that all three symptoms stay gone.
16. **Thursday's Short — "1y_vol_peak" row-count sparsity.** On 2026-07-02's data only 1 stock qualified for the active (6-Month) rotation window across all 5 rotation slots checked (0-2 typical). The row-count-adaptive table/narration code has only been exercised with n=1 so far — worth spot-checking a day where n=3-5 once one naturally occurs, to confirm the "leads/finally" flourish and table centering degrade gracefully at the higher end too (reasoned through, not yet empirically observed).
