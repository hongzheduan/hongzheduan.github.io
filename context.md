...
## What Was Done 2026-07-08/09 — "Baizora Report" Daily Market-News Short: SHIPPED (commit `16cdc07`, pushed to main)

**Goal:** a new daily video category — the Baize/白泽 mascot reads out the day's real market news headlines — generated in EN+CN alongside (weekdays) or instead of (weekends) the existing category Shorts, with its own homepage download button. Spanned three sessions (started 07-08, finished 07-09). Fully wired into `scanner.yml`, committed, and pushed to `main` — user reviewed the final CN sample and said "ok. it is good enough, wire in."

**Design iterated live, several rejected turns before landing on the final look:**
1. Background — final: **one distinct photo per story** (`News_1.png`–`News_10.png`, exactly 1:1 with `MAX_STORIES=10`), not one photo per whole video. This reversed an earlier decision (day-rotated single background) after the user reconsidered once the story count grew to 10: *"i think your original idea is great: each news with different background. now we have ten news, we just need ten background."* The hook scene alone still day-rotates across the same 10-photo pool (`news_bg_path_for_day`); each story card uses `news_bg_path_for_story(idx)` (`idx % 10 + 1`, wraps if `MAX_STORIES` ever exceeds 10). Photos are **full-bleed** (`_draw_full_bleed_bg`), not a partial-height band — an earlier partial-band layout (~0.77 aspect) forced a heavy crop on the portrait source photos (~0.563 aspect, almost matching the 1080×1920 canvas), chopping the tips off monuments/domes; user also flagged the mid-screen text placement as bad because "the building will be separated." Full-bleed + a vertical gradient scrim (`_FADE_H`) + **per-line drop-shadow text** (`centered_shadow()`) fixed both the cropping and legibility, since one global gradient can't guarantee contrast across 10 visually different photos. Final 10 photos were culled from an initial 12 (`News_7` and `News_11` removed as near-duplicates of others) after being split from 3 user-supplied grid images (`news_background.png`/`2`/`3`, all deleted post-split, no longer in the repo).
2. Mascot: went through a procedural fox (rejected), two non-brand reference images, before landing on the real Baize mascot processed into **`video/baize_mascot.png`** (RGBA, transparent bg, flood-fill bg removal, bounded-box calligraphy/seal removal, mic relabeled "NEWS"→"Baizora" via perspective warp). Animation: subtle scale-pulse ("breathing") via `bob_cycle_frames()`, `amplitude=0.015`, `cycle_frames=48`.
3. Duration: up to 10 distinct stories, whatever a given day's news genuinely supports (not forced to a fixed count) — up to ~90s. Curation must dedupe near-duplicate headlines about the same event and exclude generic listicle/opinion content (Investopedia "5 Things to Know", Motley Fool "History Says...").
4. Narration generation: **Claude Haiku 4.5** (`claude-haiku-4-5`), structured JSON output (`output_config: {format: {type: "json_schema", schema: _SCHEMA}}`) producing `{headline, narration, source}` per story, one system prompt each for EN/CN handling dedup + listicle-filtering + paraphrase in one call. Cost ~$0.007–0.02/day, negligible. Uses the same `ANTHROPIC_API_KEY` secret name as the existing chat-widget backend, but added separately as a **GitHub Actions repo secret** (Firebase Functions secrets are a different store).

**What shipped (commit `16cdc07`):**
- **`video/generate_news_report.py`** — loads `data/market_news.json`/`market_news_cn.json`, calls Haiku for curation, renders hook → up to 10 full-bleed story cards → outro via the shared `generate_video.encode()`. Skips gracefully (no file written) if the Claude call fails or too few stories survive, matching every other category's skip pattern — `scanner.yml`'s `-f` file checks handle this correctly downstream.
- **Duration estimator**, calibrated via linear regression over ~24 real `edge-tts` measurements: `estimate_duration_en = 0.218*words + 2.4`, `estimate_duration_cn = 0.132*chars + 1.6`.
- **`video/baize_mascot.png`**, **`video/covering/News_1.png`–`News_10.png`**, **`video/requirements.txt`** (+`anthropic`).
- **`.github/workflows/scanner.yml`**: weekday 4:30 PM ET cron (`30 20 * * 1-5`) now sets `IS_NEWS_RUN=True` alongside the existing `IS_VIDEO_RUN=True` → **4 videos/day** on weekdays (2 category Shorts + EN/CN news report). New weekend cron `0 20 * * 0,6` (4 PM ET) sets `IS_NEWS_RUN=True`, `IS_VIDEO_RUN=False`, `SKIP_SCAN=1`, `SKIP_EDGAR=1` → news report only, no market scan. Four new steps: generate (both langs, `|| true` so one failing doesn't block the other), upload to YouTube, commit `data/latest_video_news_en.mp4`/`_cn.mp4`/`_meta.json` for homepage download, log skips to worklog. Meta write is merge-not-overwrite (same pattern as the category-video meta fix from `cfb67c4`/`db8448a`) so a skipped language keeps its previous name/date/url instead of getting stamped with today's date.
- **`video/upload_youtube.py`**: new `news_report`/`news_report_cn` branches in `make_meta()` with EN/CN title variants.
- **`index.html`/`index_cn.html`**: second footer download link (`newsVideoWatchLink`/`newsVideoDownloadLink`), `fetch('data/latest_video_news_meta.json')` block mirroring the existing category-video pattern exactly (YouTube oEmbed title fetch, download href).
- Verified via structural smoke tests only (stubbed `curate_stories()`, both EN and 4-story then 10-story-config CN renders, frames extracted and visually inspected — hook and card frames both crisp, no cropping, no low-contrast text). **The real Haiku API call has never been exercised end-to-end** — first real test happens on the next scheduled `scanner.yml` run (or a manual `workflow_dispatch`) now that the code is on `main`.

**Confirmed before pushing:** diffed every staged file line-by-line — all changes across `scanner.yml`, `index.html`, `index_cn.html`, `upload_youtube.py`, `requirements.txt` were purely additive (only removed lines were the expected `SKIP_SCAN` gate additions to 4 pre-existing `if:` conditions); nothing unrelated was touched.

**Still open / worth watching:**
1. Confirm `ANTHROPIC_API_KEY` is actually present as a GitHub Actions repo secret (not just Firebase Functions) before the first live run — never independently verified in this session, only that the user said they'd added it.
2. First real production run will be the true test of: structured-output parsing against real headlines, narration duration accuracy against the calibrated (not measured-for-real-content) estimator, and whether Haiku's dedup/listicle-filtering holds up on a live day's `market_news*.json`.
3. User said the design "may [get] further adjust[ed] in some details" — treat the current full-bleed/per-story-background/drop-shadow-text layout as the shipped baseline, not necessarily final.
